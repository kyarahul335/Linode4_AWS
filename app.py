...
def create_linode_instance(instance_number, results, image, region, instance_type, root_password, token, user_key):
    try:
        ...
        response = requests.post(
            ...
            timeout=30  # 30 seconds timeout for the API request
        )
...
def create_instances():
    ...
    errors = []
    try:
        # Process instances in batches of 5
        batch_size = 5
        ...
        for batch in range(total_batches):
            ...
            with ThreadPoolExecutor(max_workers=batch_size) as executor:
                ...
                timeout_per_instance = 60  # 60 seconds per instance
                batch_timeout = timeout_per_instance * len(batch_instances)
                done, not_done = wait(futures, timeout=batch_timeout)
                
                if not_done:
                    # Cancel any pending tasks in current batch
                    for future in not_done:
                        future.cancel()
                    errors.append(f"Batch {batch + 1}: Operation timed out while creating instances {start_idx}-{end_idx-1}")
                    continue
